...
namespace HLTools
{
    ...
    public class WAD3Loader
    {
        ...
        public struct CharInfo
        {
            ...
            public override string ToString()
            {
                return string.Format("偏移: {0:X} ,  宽度: {1:X}", StartOffset, CharWidth);
            }
        }
       ...
       public class QFont
       {
        ...
        public void LoadFile(string inputFile)
        {
            ...
            {
                Close();
                throw new HLToolsUnsupportedFile("无效或不支持的WAD文件！");
            }
            ...
            if (index > -1 && index < LumpsInfo.Count)
            {
                ...
                if (type == 0x40 || type == 0x42 || type == 0x43 || type == 0x46) //Supported types
                {
                    ...
                    if (width > MaxTextureWidth || height > MaxTextureHeight)
                        throw new TextureDimensionException("纹理宽度或高度超出最大尺寸！");

                    if (width == 0 || height == 0)
                        throw new TextureDimensionException("纹理宽度和高度必须大于0！");
                ...
                {
                    throw new HLToolsUnsupportedFile("WAD中不支持的 (0x" + LumpsInfo[index].Type.ToString("X") + ") 块体（纹理）类型！");
                }
            }
            else
            {
        ...
        public static int ExtractWadFromBsp(string inputBspFilename, string outputWadFilename)
        {
            ...
            if (Path.GetExtension(inputBspFilename) != ".bsp")
                throw new ArgumentException("导入的文件必须是BSP格式！");
