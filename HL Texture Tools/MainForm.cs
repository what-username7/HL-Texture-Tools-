    ...
    public partial class MainForm : Form
    {
        ...
        private void ReloadCurrentImage()
        {
            ...
            if (currentFileType == HLFileType.Wad)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    SetErrorImageMessage("错误: " + ex.Message);
                }
            }
        ...
        private void OpenFile(string filename)
        {
            try
            {
                ...
                switch (Path.GetExtension(filename).ToLower())
                {
                    ...
                    case ".spr":
                        ...
                        break;
                    default:
                        SetErrorImageMessage(string.Format("错误: 未知文件类型！ [{0}]", Path.GetFileName(filename)));
                        return;
                }
            ...
            catch (Exception ex)
            {
                ...
                totalLbl.Text = "-1";
                SetErrorImageMessage(string.Format("错误: 打开文件时！ [{0}]\n详细信息: {1}", Path.GetFileName(filename), ex.Message));
            ...
            if (folderDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                else if (currentFileType == HLFileType.Sprite)
                {
                    ...
                    for (int i = 0; i < frames.Length; i++)
                    {
                        ...
                        {
                            frames[i].Image.Save(Path.Combine(folderDialog.SelectedPath, "帧数 " + i.ToString() + ext), format);
                        }
                        progressBar.Value++;
                        Application.DoEvents();
                    }
        ...
        private void fixTextureNameToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (listBox1.SelectedItem != null)
            {
                ...
                InputDialog inputDialog = new InputDialog("名称", "输入纹理的新名称:", val, 15);
                if (inputDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        MessageBox.Show("纹理名称不能为空!", "纹理名称", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
            }
        }
        ...
        private void associateToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (!UAC.IsAdmin())
            {
                ...
                {
                    MessageBox.Show("没有管理员权限无法关联扩展!", "错误", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                }
            }
            else
            {

                DialogResult dRes = MessageBox.Show("将*.wad和*.spr文件与该程序关联?", "文件关联",
                ...
                if (dRes == DialogResult.Yes)
                {
                    ...
                    paiWad.Create(new ProgramVerb("打开", Application.ExecutablePath + " \"%1\""));
                    paiSpr.Create(new ProgramVerb("打开", Application.ExecutablePath + " \"%1\""));
                }
            }
        }
